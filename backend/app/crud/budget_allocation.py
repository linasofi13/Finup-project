...
def get_budget_allocation(db: Session, allocation_id: int):
    return (
        ...
        .options(joinedload(BudgetAllocation.evc))
        .filter(BudgetAllocation.id == allocation_id)
        .first()
    )
...
def get_budget_allocations_by_pocket(db: Session, budget_pocket_id: int):
    return (
        db.query(BudgetAllocation)
        .options(joinedload(BudgetAllocation.evc))
        .filter(BudgetAllocation.budget_pocket_id == budget_pocket_id)
        .all()
    )
...
def get_budget_allocations_by_evc(db: Session, evc_id: int):
    return (
        db.query(BudgetAllocation)
        .options(joinedload(BudgetAllocation.evc))
        .filter(BudgetAllocation.evc_id == evc_id)
        .all()
    )
...
def create_budget_allocation(db: Session, allocation: BudgetAllocationCreate):
    ...
    budget_pocket = (
        ...
        .filter(BudgetPocket.id == allocation.budget_pocket_id)
        .first()
    )
    ...
    if allocation.is_total_allocation:
        ...
        if remaining <= 0:
            raise HTTPException(
                status_code=400, detail="No remaining budget to allocate"
            )
    ...
    if new_total > budget_pocket.agreed_value:
        raise HTTPException(
            status_code=400, detail="Allocation would exceed agreed budget value"
        )
    ...
    return (
        db.query(BudgetAllocation)
        .options(joinedload(BudgetAllocation.evc))
        .filter(BudgetAllocation.id == db_allocation.id)
        .first()
    )
...
def update_budget_allocation(
    ...
    budget_pocket = (
        db.query(BudgetPocket)
        .filter(BudgetPocket.id == db_allocation.budget_pocket_id)
        .first()
    )
    if not budget_pocket:
        raise HTTPException(status_code=404, detail="Budget pocket not found")
    ...
    new_value = (
        ...
        if allocation.allocated_value is not None
        else old_value
    )

    # If this is a total allocation, set the allocated_value to the remaining amount
    if allocation.is_total_allocation:
        remaining = budget_pocket.agreed_value - (
            budget_pocket.total_allocated - old_value
        )
        if remaining <= 0:
            raise HTTPException(
                status_code=400, detail="No remaining budget to allocate"
