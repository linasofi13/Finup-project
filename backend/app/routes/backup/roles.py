...
async def update_role(
    role_id: int, role_update_data: RoleUpdate, db: Session = Depends(get_db)
):
    db_role = role_service.update_role(
        db=db, role_id=role_id, role_update_data=role_update_data
    )
    if not db_role:
