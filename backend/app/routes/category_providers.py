...
from app.schemas.category_provider import (
    CategoryProvider,
...
async def create_category_provider(
    category_provider_data: CategoryProviderCreate, db: Session = Depends(get_db)
):
    return category_provider_service.create_category_provider(
        db, category_provider_data
    )
...
@router.put(
    "/{category_provider_id}", response_model=CategoryProviderResponse, tags=[tag_name]
)
async def update_category_provider(
    category_provider_id: int,
    category_provider_data: CategoryProviderUpdate,
    db: Session = Depends(get_db),
):
    db_category_provider = category_provider_service.update_category_provider(
        db, category_provider_id, category_provider_data
    )
...
async def delete_category_provider(
    category_provider_id: int, db: Session = Depends(get_db)
):
    category_provider = category_provider_service.get_category_provider_by_id(
        db, category_provider_id
    )
