...
from app.schemas.category_role import CategoryRole, CategoryRoleCreate, CategoryRoleResponse, CategoryRoleUpdate
...
async def create_category_role(category_role_data: CategoryRoleCreate, db: Session = Depends(get_db)):
    return category_role_service.create_category_role(db, category_role_data)

@router.get("/category-roles/", response_model=List[CategoryRoleResponse], tags=[tag_name])
async def get_category_roles(db: Session = Depends(get_db)):
    return category_role_service.get_category_roles(db)

@router.put("/category-roles/{category_role_id}", response_model=CategoryRoleResponse, tags=[tag_name])
async def update_category_role(category_role_id: int, category_role_data: CategoryRoleUpdate, db: Session = Depends(get_db)):
    db_category_role= category_role_service.update_category_role(db, category_role_id, category_role_data)
    if not db_category_role:
        raise HTTPException(status_code=404, detail="CategoryRole not found")
    return db_category_role

@router.delete("/category-roles/{category_role_id}", response_model=CategoryRoleResponse, tags=[tag_name])
