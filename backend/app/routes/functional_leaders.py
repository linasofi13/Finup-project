...
from app.schemas.functional_leader import FunctionalLeaderCreate, FunctionalLeaderResponse
...
async def update__functional_leader(
    functional_leader_id: int, functional_leader_data: FunctionalLeaderCreate, db: Session = Depends(get_db)):
    db_functional_leader = functional_leader_service.update_functional_leader(db, functional_leader_id, functional_leader_data)
...
@router.delete("/functional-leaders/{functional_leader_id}", response_model=FunctionalLeaderResponse, tags=[tag_name])
async def delete_functional_leader(functional_leader_id: int, db: Session = Depends(get_db)):
    db_functional_leader = functional_leader_service.delete_functional_leader(db, functional_leader_id)
    if not db_functional_leader:
        raise HTTPException(status_code=404, detail="Functional Leader not found")
    return db_functional_leader