...
from app.schemas.role_provider import RoleProvider, RoleProviderCreate, RoleProviderResponse, RoleProviderUpdate
...
async def create_role_provider(role_provider_data: RoleProviderCreate, db: Session = Depends(get_db)):
...
async def update_role_provider(role_provider_id: int, role_provider_data: RoleProviderUpdate, db: Session = Depends(get_db)):
    db_role_provider= role_provider_service.update_role_provider(db, role_provider_id, role_provider_data)
    if not db_role_provider:
        raise HTTPException(status_code=404, detail="RoleProvider not found")
    return db_role_provider

@router.delete("/{role_provider_id}", response_model=RoleProviderResponse, tags=[tag_name])
