...
def create_category_provider(db: Session, category_provider_data: CategoryProviderCreate):
...
def get_category_provider_by_id(db: Session, category_provider_id: int):
    return db.query(CategoryProvider).filter(CategoryProvider.id == category_provider_id).first()
...
def update_category_provider(db: Session, category_provider_id: int, category_provider_update_data: CategoryProviderUpdate):
    db_category_provider = get_category_provider_by_id(db, category_provider_id)
    if db_category_provider:
        for key, value in category_provider_update_data.dict(exclude_unset=True).items():
