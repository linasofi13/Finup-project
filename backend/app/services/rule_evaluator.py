    ...
    try:
        ...
        if changed_table == "evc_financial" and changed_id:
            ...
            if quarter_info:
                ...
                if recent_notifications > 0:
                    print(f"Skipping duplicate notifications for EVC quarter {quarter_info.evc_q_id} - already notified in the last 5 minutes")
                    return
        ...
        for rule in rules:
            ...
            try:
                ...
                if table == "evc_q" and (field == "spent_percentage" or field == "spent_budget"):
                    try:
                        ...
                        if changed_id:
                            ...
                            if q and q.allocated_budget > 0:
                                ...
                                spent_percentage = (spent_budget / q.allocated_budget) * 100
                                print(f"Quarter ID {q.id}: Budget: {q.allocated_budget}, Spent: {spent_budget} ({spent_percentage:.1f}%)")
                                ...
                                if field == "spent_percentage":
                                    if comparison == ">=" and spent_percentage >= threshold:
                                        should_notify = True
                                    elif comparison == ">" and spent_percentage > threshold:
                                        should_notify = True
                                    elif comparison == "==" and spent_percentage == threshold:
                                        should_notify = True
                                    elif comparison == "<=" and spent_percentage <= threshold:
                                ...
                                if should_notify:
                                    ...
                                    if field == "spent_percentage":
                                        ...
                                        higher_reported = [t for t in reported_thresholds[q.id][field] if t > threshold]
                                        if higher_reported:
                                            continue
                                            
                                        # Skip if it's a very small change from previously reported (less than 5%)
                                        close_reported = [t for t in reported_thresholds[q.id][field] 
                        ...
                        print(f"[evaluate_rules] Error handling calculated field {field}: {e}")
                        continue
        ...
        if notifications_to_add:
            ...
            for idx, note in enumerate(notifications_to_add):
                print(f"Notification {idx+1}: {note['message'][:50]}... ({note['type']})")
            ...
            print(f"After filtering, {len(filtered_notifications)} unique notifications remain")
            ...
            if filtered_notifications:
                try:
                    ...
                    try:
                        ...
                        notif_db.commit()
                        print(f"Successfully added {len(filtered_notifications)} notifications to database")
                    except Exception as e:
                        notif_db.rollback()
