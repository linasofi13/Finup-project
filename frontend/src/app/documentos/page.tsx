...
export default function DocumentosPage() {
  ...
  const fetchDocumentos = async () => {
    try {
      const response = await axios.get("http://127.0.0.1:8000/provider-documents/");
  ...
  const handleFiltroInput = (e) => {
    if (filtroCampo === "uploaded_at" && isNaN(Date.parse(e.target.value))) return;
  ...
  const documentosPaginados = documentosFiltrados.slice((pagina - 1) * porPagina, pagina * porPagina);
        ...
        <table className="min-w-full bg-white border border-gray-300 rounded-lg">
          ...
          <tbody>
            {documentosPaginados.map((doc) => (
              <tr key={doc.id} className="border text-gray-700 hover:bg-yellow-100">
                ...
                <td className="p-3 border">{doc.provider?.name || "Sin nombre"}</td>
