      ...
      evc.evc_qs.reduce((sum, q) => {
       ...
       const quarterPercentage =
        q.allocated_budget > 0
         ? Math.min(
            Math.round(((q.total_spendings || 0) / q.allocated_budget) * 100),
            100
           )
         : 0;
...
function QuarterCard({
 ...
 >;
 uploadStatus: {
  [quarterId: number]: { uploading: boolean; error?: string; success?: string };
 };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
     ...
     <div className="bg-white rounded-lg p-6 max-w-md relative z-10">
      ...
      <p className="mb-4 text-sm text-gray-600">
       Registrar este gasto hará que se exceda el presupuesto asignado para este
       quarter.
      </p>
    ...
    <div className="flex justify-between items-center mb-1">
     ...
     <span className="text-sm font-bold text-gray-900">
      ...
      {quarter.allocated_budget > 0
       ? Math.min(
          Math.round(
           ((quarter.total_spendings || 0) / quarter.allocated_budget) * 100
          ),
          100
         )
       : 0}
     ...
     {uploading ? (
      ...
      <>
       ...
       <span className="text-blue-700">
        Arrastra y suelta aquí tu factura PDF o haz clic para seleccionarla
       </span>
       <input
        type="file"
        accept=".pdf"
...
function EvcsPage() {
 ...
 const [providerPriceRange, setProviderPriceRange] = useState<[number, number]>(
  [0, 15000]
 );
 ...
 const loadEntornosData = async () => {
  try {
   const response = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/entornos/entornos/`);
   const data: { [key: number]: string } = {};
 ...
 const fetchEvcs = async () => {
  try {
   const response = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/evcs/`);
 ...
 const fetchAvailableEntornos = async () => {
  try {
   const resp = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/entornos/entornos/`);
 ...
 const fetchAvailableProviders = async () => {
  try {
   const resp = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/providers/providers/`);
 ...
 const createEvc = async () => {
  try {
   const response = await axios.post(`${process.env.NEXT_PUBLIC_API_BASE_URL}/evcs/`, {
    name: newEvc.name,
 ...
 const createQuarter = async (evcId: number) => {
  try {
   ...
   const response = await axios.post(`${process.env.NEXT_PUBLIC_API_BASE_URL}/evc-qs/evc_qs/`, {
    evc_id: evcId,
    year,
    q,
    allocated_budget,
   ...
   const updatedEvc = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/evcs/${evcId}`);
 ...
 const deleteEvc = async (evcId: number) => {
  try {
   const token = Cookies.get("auth_token");
   await axios.delete(`${process.env.NEXT_PUBLIC_API_BASE_URL}/evcs/${evcId}`, {
    headers: {
     Authorization: `Bearer ${token}`,
    },
   });
 ...
 const updateEvcField = async (evcId: number, field: string, value: any) => {
  try {
   ...
   const response = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/evcs/${evcId}`, {
    headers: {
     Authorization: `Bearer ${token}`,
    },
   });
 ...
 const toggleEvcStatus = async (evc: EVC) => {
  try {
   ...
   setEvcs((prevEvcs) =>
    prevEvcs.map((e) => (e.id === evc.id ? { ...e, status: !e.status } : e))
   );
     ...
     <label className="flex items-center cursor-pointer select-none">
      <input
       ...
       checked={
        ...
        (filteredEvcs.length > 0 ? filteredEvcs.length : evcs.length) > 0
       }
       onChange={(e) => {
        if (e.target.checked) {
         setSelectedEvcsForDelete(
          (filteredEvcs.length > 0 ? filteredEvcs : evcs).map((evc) => evc.id)
         );
        } else {
         setSelectedEvcsForDelete([]);
        }
     ...
     <div className="relative bg-white rounded-2xl shadow-2xl p-8 w-full max-w-6xl max-h-[90vh] overflow-y-auto flex flex-col">
      ...
      <div className="mb-6 pr-10">
       <h2 className="text-3xl font-extrabold text-gray-900 mb-1">
        Gestión de Quarters -{" "}
        <span className="font-bold text-blue-700">{selectedEvc.name}</span>
       </h2>
      </div>
          ...
          <ul className="space-y-2">
           {selectedEvc.evc_qs.map((quarter) => (
            <li key={quarter.id} className="flex items-center gap-2">
             <span className="font-semibold">Año:</span> {quarter.year}{" "}
          ...
          {selectedEvc.evc_qs.map((quarter) => (
           ...
           >
            <div className="flex flex-wrap gap-4 mb-2 items-center">
             ...
             <div className="text-base font-semibold text-gray-700">
              Quarter:{" "}
              <span className="font-bold text-gray-900">Q{quarter.q}</span>
             </div>
             <button
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-start">
     ...
     >
      ...
      <div className="w-full overflow-y-auto" style={{ maxHeight: "260px" }}>
       {getFilteredProviders(providerFilterModal.quarterId ?? 0).length ===
       0 ? (
            ...
            <button
             ...
             onClick={() => {
              if (providerFilterModal.quarterId != null) {
               setProviderSelections((prev) => ({
                ...prev,
                [providerFilterModal.quarterId!]: provider.id.toString(),
               }));
              }
              setProviderFilterModal({
               quarterId: null,
