...
export default function ProveedoresPage() {
  ...
  const filteredProviders = proveedores.filter((prov) => {
    ...
    if (!prov.role.toLowerCase().includes(filters.role.toLowerCase())) return false;
  ...
  const handleDeleteSelected = async () => {
    if (!window.confirm("¿Confirma la eliminación de los registros seleccionados?")) return;
    try {
  ...
  const handleFileUpload = async (event) => {
    ...
    if (!file.name.endsWith('.xlsx') && !file.name.endsWith('.xls')) {
      alert("Formato de archivo no soportado. Solo se aceptan archivos .xlsx o .xls");
      return;
    }
    ...
    reader.onload = async (e) => {
      try {
        ...
        const requiredColumns = ["Nombre", "Rol", "Proveedor", "País", "Costo USD", "Categoría", "Línea", "Correo"];
        const sheetColumns = Object.keys(jsonData[0]);
        const missingColumns = requiredColumns.filter(col => !sheetColumns.includes(col));
        if (missingColumns.length > 0) {
          alert(`El archivo Excel no tiene las siguientes columnas requeridas: ${missingColumns.join(", ")}`);
...
>
    <FaFileUpload className='mr-2' /> Exportar a Excel
            ...
            <thead>
              <tr className='bg-gray-100 text-gray-700'>
                <th className='p-3 border'>
                  <input 
                    type="checkbox" 
                    checked={filteredProviders.length > 0 && selectedIds.length === filteredProviders.length} 
            ...
            <tbody>
              {filteredProviders.map((proveedor) => (
                proveedor.id === editingProveedor?.id ? (
                  <tr key={proveedor.id} className='border bg-yellow-50'>
                    ...
                    <td className='p-3 border'>
                      <input 
                        ...
                        onChange={(e) => setEditingProveedor({ ...editingProveedor, name: e.target.value })} 
                        className="w-full border p-1 rounded" 
                      />
                    </td>
                    <td className='p-3 border'>
                      <input 
                        ...
                        onChange={(e) => setEditingProveedor({ ...editingProveedor, role: e.target.value })} 
                        className="w-full border p-1 rounded" 
                      />
                    </td>
                    <td className='p-3 border'>
                      <input 
                        ...
                        onChange={(e) => setEditingProveedor({ ...editingProveedor, company: e.target.value })} 
                        className="w-full border p-1 rounded" 
                      />
                    </td>
                    <td className='p-3 border'>
                      <input 
                        ...
                        onChange={(e) => setEditingProveedor({ ...editingProveedor, country: e.target.value })} 
                        className="w-full border p-1 rounded" 
                      />
                    </td>
                    <td className='p-3 border'>
                      <input 
                        ...
                        onChange={(e) => setEditingProveedor({ ...editingProveedor, cost_usd: e.target.value })} 
                        className="w-full border p-1 rounded" 
                      />
                    </td>
                    <td className='p-3 border'>
                      <input 
                        ...
                        onChange={(e) => setEditingProveedor({ ...editingProveedor, category: e.target.value })} 
                        className="w-full border p-1 rounded" 
                      />
                    </td>
                    <td className='p-3 border'>
                      <input 
                        ...
                        onChange={(e) => setEditingProveedor({ ...editingProveedor, line: e.target.value })} 
                        className="w-full border p-1 rounded" 
                      />
                    </td>
                    <td className='p-3 border'>
                  ...
                  <tr key={proveedor.id} className='border text-gray-700 hover:bg-yellow-100'>
                    <td className='p-3 border'>
                      <input 
                        type="checkbox" 
        ...
        <div className='mt-8 p-6 bg-gray-50 rounded-lg shadow-md w-full'>
          ...
          <p className="text-sm text-gray-600 mb-2">Formatos soportados: .xlsx, .xls</p>
          <input 
            type='file' 
            ref={fileInputRef} 
            className='hidden' 
          ...
          >
            <FaFileUpload className='mr-2' /> {loading ? "Cargando..." : "Subir Archivo"}
          </button>
        </div>
      </div>
      ...
      <div className="mt-8 space-y-4">
          ...
          <div className="mt-4">
            ...
            {docList.length === 0 ? (
              ...
              <ul className="space-y-2">
                {docList.map((doc) => {
                  const prov = proveedores.find(p => p.id === parseInt(doc.providerId, 10));
                  return (
        ...
        <div className="bg-white p-4 rounded-lg shadow-md">
          ...
          <p className="text-sm text-gray-600">
            Aquí podrías mostrar la información de contratos, fechas, renovaciones, etc.
          </p>
        </div>
      </div>
      ...
      {showPreviewModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-10">
          ...
          >
            <h2 className="text-xl font-bold mb-4">Previsualizar Carga Masiva</h2>
            <div className="overflow-x-auto mb-4">
              <table className="min-w-full border border-gray-300 rounded-lg">
                <thead>
                  <tr className='bg-gray-100 text-gray-700'>
                    {["Nombre", "Rol", "Proveedor", "País", "Costo USD", "Categoría", "Línea", "Correo"].map((header) => (
                ...
                <tbody>
                  {filePreview.map((item, idx) => (
                    <tr key={idx} className="border text-gray-700 hover:bg-yellow-100">
