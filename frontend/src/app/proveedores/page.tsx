...
import { FaCheck, FaPlus, FaTrash, FaFileUpload, FaTimes } from 'react-icons/fa';
...
export default function ProveedoresPage() {
  ...
  const agregarRegistro = () => {
    setNuevosProveedores((prev) => [
      ...prev,
      { name: '', role: '', company: '', country: '', cost_usd: '', category: '', line: '', email: '' },
    ]);
  };
  ...
  return (
    <div className='p-6 mt-20 bg-white shadow-md rounded-lg flex flex-col'>
      <div className='flex justify-between items-center mb-4'>
        <button onClick={agregarRegistro} className='px-4 py-2 bg-yellow-500 text-white rounded-md hover:bg-yellow-600 flex items-center'>
          <FaPlus className='mr-2' /> Añadir Registro
        </button>
      </div>

      <div className='overflow-x-auto'>
        <table ref={tableRef} className='min-w-full bg-white border border-gray-300 rounded-lg'>
          <thead>
          ...
          <tbody>
            ...
            {nuevosProveedores.map((nuevo, index) => (
              <tr key={`nuevo-${index}`} className='border text-gray-700 bg-yellow-50'>
                ...
                <td className='p-3 border flex space-x-2'>
                  <FaCheck className='text-green-500 cursor-pointer' onClick={() => guardarNuevoProveedor(index)} />
                </td>
              </tr>
            ))}
          </tbody>
